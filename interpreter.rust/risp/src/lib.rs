...
pub fn eval(exp: &RispExp, env: &mut RispEnv) -> Result<RispExp, RispErr> {
    ...
    match exp {
        RispExp::Void => Ok(exp.clone()),
        RispExp::Symbol(k) => {
            env_get(k, env).ok_or(RispErr::Reason(format!("unexpected symbol k='{}'", k)))
        }
        // RispExp::Symbol(k) => Ok(env_get(k, env).unwrap_or(exp.clone())),
        // RispExp::Symbol(_a) => Ok(exp.clone()),
        ...
        RispExp::Func(_) => Err(RispErr::Reason("unexpected func form".to_string())),
        RispExp::Lambda(_) => Err(RispErr::Reason("unexpected lambda form".to_string())),
    }
}
